...
export function LastStep(status, step) {
  ...
  const $divCard = d.createElement('div'); $divCard.className = 'card t-center';

  const $cardDivData = d.createElement('div'); $cardDivData.className = 'card__data';

  const $cardDivInputs = d.createElement('div'); $cardDivInputs.className = 'card__btn flex-jus-center'
  const $cardInputPrev = d.createElement('button');
  $cardInputPrev.type = 'button';

  if (status == 'ok') {
    $cardDivData.innerHTML = `<h3>Registro Exitoso</h3><h1>👍</h1>`
    $cardInputPrev.setAttribute('data-close', 'close');
    $cardInputPrev.innerText = 'Finalizar'; $cardInputPrev.className = 'btn btn__send';
  } else {
    ...
    $cardInputPrev.innerText = 'Regresar'; $cardInputPrev.className = 'btn btn__prev';
  }
